...
router.post('/', async (req, res) => {
    try {
        ...
        const invitations = await Promise.all(invitedEmails.map((email) => prisma.goalInvitation.create({
            data: {
                sharedGoalId: sharedGoal.id,
                invitedEmail: email,
...
router.get('/', async (req, res) => {
    try {
        ...
        sharedGoals.forEach((sg) => {
            console.log(`  Goal: ${sg.goal.title}, Members: ${sg.members.map((m) => m.user.email).join(', ')}`);
        });
...
//# sourceMappingURL=sharedGoal.routes.js.map